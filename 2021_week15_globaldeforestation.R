...
p <- d_soy %>% 
  ...
  geom_richtext(aes(label = "<span style='color:black'>Between 1962 and 2013,<br>global soybean use increased<br>from 23 million tonnes to<br>
                             **<span style='face:bold;'>255 million tonnes.</span>**",
                    x = 1963, y = 225000000), label.color = NA, fill = NA, hjust = 0, lineheight = 0.34, size = 30) +
  scale_fill_identity() + 
  scale_colour_identity() + 
  scale_y_continuous(breaks = seq(0, 250000000, 50000000), labels = seq(0, 250, 50), expand = c(0, 0)) + 
  scale_x_continuous(breaks = seq(1960, 2010, 5), labels = seq(1960, 2010, 5), limits = c(1962, 2018), expand = c(0, 0)) + 
...
ggsave(p, dpi = 300, width = 10, height = 10, units = "in", filename = "2021_week15_globaldeforestation.png", device = "png")
